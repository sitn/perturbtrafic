...
export class ContactEditionComponent implements OnInit, OnDestroy {
  ...
  saveContact() {
    this.errorMessage = null;
    const force: boolean = this.warningDisplayed;
    this.warningDisplayed = false;
    if (this.contactEditionFormGroup.valid) {
      if (this.mode === 'NEW') {
        this.apiService.saveContact(this.contactEditionFormGroup.value, force).subscribe(res => {
          this.warningDisplayed = false;
          if (!res.error) {
            this.navigationService.closeNewContactDialog(true);
            if (this.dropdownOrigin) {
              this.dropdownOrigin.value = res.id;
            }
            this.dropDownService.getContacts(res.id);
            this.close();
          } else {
  ...
  private setSubscriptions(): void {
    ...
    this.subscriptions.push(
      this.navigationService.openNewContactDialog$.subscribe((val: { mode: string, contact: IContact, dropdownOrigin?: any }) => {
        this.contactEditionFormGroup.reset();
        this.mode = val.mode;
        this.contact = val.contact;
        if (val.dropdownOrigin) {
          this.dropdownOrigin = val.dropdownOrigin;
        }
        if (this.contact) {
